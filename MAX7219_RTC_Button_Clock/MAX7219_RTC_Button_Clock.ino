...
#define SLIDE_DELAY 50                   // The time in milliseconds for the slide effect per character in slide mode. Make this higher for a slower effect
...
void printver() {
  ...
  i = 0;
  char name1[12] = "DIGITAL";
  while (name1[i]) {
    puttingchar((i * 4), 1, name1[i]);
    delay(35);
    i++;
  }
  delay(700);
  ...
  i = 0;
  byte name4[12] = "ABHIRUP";
  while (name4[i]) {
    puttingchar((i * 4), 1, name4[i]);
    delay(35);
    i++;
  }
  delay(700);
  ...
  i = 0;
  byte name6[12] = "ROBOTICS";
  while (name6[i]) {
    puttingchar((i * 4), 1, name6[i]);
    delay(35);
    i++;
  }
  delay(700);
  ...
  while (hello_message[i]) {
    puttingchar((i * 4), 1, hello_message[i]);
    delay(35);
    i++;
  }
  delay(700);
...
void puttingchar(byte x, byte y, char c)
{
  byte dots;
  if (c >= 'A' && c <= 'Z' || (c >= 'a' && c <= 'z') ) {
...
void small_mode() {
  ...
  while (run_mode()) {
    ...
    if (secs != old_secs) {
      ...
      puttingchar( 20, 1, ':'); //seconds colon
      puttingchar( 24, 1, buffer[0]); //seconds
      puttingchar( 28, 1, buffer[1]); //seconds
      old_secs = secs;
    }
...
void word_clock() {
  ...
  while (run_mode()) {
    ...
    byte i = 0;
    while (str_a[i]) {
      puttingchar((i * 4) + offset_top, 1, str_a[i]);
      i++;
    }
    ...
    i = 0;
    while (str_b[i]) {
      puttingchar((i * 4) + offset_top, 1, str_b[i]);
      i++;
    }
    ...
    i = 0;
    while (str_c[i]) {
      puttingchar((i * 4) + offset_top, 1, str_c[i]);
      i++;
    }
    counter = 1000;
    while (counter > 0){
...
void display_date()
{
  ...
  {
    puttingchar((i*4) + offset , 1, daysfull[dow][i]); 
    i++;
  }
  delay(1000);
  fade_down();
  ...
  puttingchar(-4+offset, 1, buffer[0]);
  ...
  if (date > 9){
    suffixposx = 8;
    puttingchar(0+offset, 1, buffer[1]);
    offset = 8; //offset to centre text if 4 chars
  }

  //print the 2 suffix characters
  puttingchar(suffixposx+offset, 1, suffix[s][0]); 
  puttingchar(suffixposx+4+offset, 1, suffix[s][1]); 
  ...
  {  
    puttingchar((i*4) +offset, 1, monthnames[month][i]); 
    i++; 
  }
  ...
  offset = 10; //offset to centre text if 3 chars - e.g. 3rd
  puttingchar(-2+offset, 1, bufferx[0]);
  puttingchar(2+offset, 1, bufferx[1]);
  puttingchar(6+offset, 1, bufferx[2]);
  puttingchar(10+offset, 1, bufferx[3]);
...
void set_intensity() {
  ...
  while (!buttonA.uniquePress()) {
    ...
    while (buttonC.isPressed()) {
      ...
      plot((intensity*2)+3,6,0);
      plot((intensity*2)+4,6,0);
      plot((intensity*2)+3,7,0);
      plot((intensity*2)+4,7,0); 
...
void set_time() {
  ...
  set_yr   = set_value(4, set_yr, 1, 3000);
...
int set_value(byte message, int current_value, int reset_value, int rollover_limit){
  ...
  {
    puttingchar(i*4 , 1, messages[message][i]); 
    i++;
  }
  ...
  puttingchar(0 , 1, buffer[0]); 
  puttingchar(4 , 1, buffer[1]); 
  puttingchar(8 , 1, buffer[2]); 
  puttingchar(12, 1, buffer[3]); 
  ...
  while (!buttonA.uniquePress()) {
    ...
    while (buttonC.isPressed()){
      ...
      if(current_value > reset_value) { 
        ...
        current_value = rollover_limit-1;
      }
      //print the new value
      itoa(current_value, buffer ,10);
      puttingchar(0 , 1, buffer[0]); 
